...
def test_signing_is_wired_into_publishing_once_a_key_exists():
    ...
    if DU.UPDATE_PUBLIC_KEYS:
        ...
        #Ключа нет — подпись не обязательна, и это состояние описано честно. Но если
        #выкладка вдруг начала подписывать, значит ключ завели где-то ещё: список
        #открытых ключей обязан был обновиться в тот же заход.
        assert not calls_signer, (
            "publish_desktop_update.sh подписывает манифест, а UPDATE_PUBLIC_KEYS пуст — "
            "подпись некому проверить, и это расхождение надо закрыть в том же заходе")
